...
export default function TokenHistoryList({
  ...
  if (transactions.length === 0) {
    return <div style={styles.empty}>{resolvedEmptyMessage}</div>;
  }
  ...
  return (
    <div style={styles.list}>
      {transactions.map((transaction) => {
        ...
        const hasReferenceContext = transaction.requestId || transaction.lessonId;
        const reason = String(transaction.reason || '').trim();
        ...
        return (
          <div key={transaction.id} style={styles.card} dir={isHe ? 'rtl' : 'ltr'}>
            ...
            {showRawReason && reason && (
              <div style={styles.rawReasonRow}>
                <span style={styles.rawReasonLabel}>{isHe ? 'סיבת מערכת' : 'System reason'}</span>
                <span style={styles.rawReasonText}>{reason}</span>
              </div>
            )}

            {(hasLessonContext || hasReferenceContext) && (
              <div style={styles.contextGrid}>
                ...
                {transaction.requestId && (
                  <div style={styles.contextItem}>
                    <span style={styles.contextLabel}>{isHe ? 'בקשה' : 'Request'}</span>
                    <span style={styles.contextValue}>#{transaction.requestId}</span>
                  </div>
                )}
                {transaction.lessonId && (
                  <div style={styles.contextItem}>
                    <span style={styles.contextLabel}>{isHe ? 'שיעור' : 'Lesson'}</span>
                    <span style={styles.contextValue}>#{transaction.lessonId}</span>
                  </div>
                )}
              </div>
            )}
